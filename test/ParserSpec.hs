...
abbreviationsSpec = describe "Parsing abbreviations" $ do
  ...
  itShPrs "unquote empty list"    ",()"      (Unquote (DList [])) -- shouldn't be possible at level above quote
  itShPrs "quote list"            "'(+ 1 2)" (Quote (DList []))
  itShPrs "quasiquote list"       "`(+ 1 2)" (Quasiquote (DList []))
  itShPrs "unquote list"          ",(+ 1 2)" (Unquote (DList []))
...
compoundSpec = describe "Parsing some combinations" $ do
  ...
  itShPrs "addition"
          "(+ 1 2)"
          (List [Variable "+", lNum (Integral 1), lNum (Integral 2)])
  itShPrs "subtraction"
          "(- 1 2)"
          (List [Variable "-", lNum (Integral 1), lNum (Integral 2)])
  itShPrs
    ...
    (List
      [ Variable "-"
      , List [Variable "+", lNum (Integral 1), lNum (Integral 1)]
      , lNum (Integral 2)
      ]
    )
  itShPrs
  ...
  itShPrs
    ...
    "`(,(+ 1 2))"
    (Quasiquote
      (DRestricted (List [Variable "+", lNum (Integral 1), lNum (Integral 2)]))
    )
